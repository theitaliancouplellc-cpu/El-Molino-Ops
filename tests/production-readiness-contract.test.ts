...
const scheduleI18n=read('lib/i18n-schedule.ts');
...
test('employee schedule remains employee-scoped, server-authoritative, and offline-safe',()=>{
  ...
  assert.match(schedule,/tx\.connectionSaved/);
  assert.match(schedule,/\{tx\.savedSchedule\}/);
  assert.match(schedule,/\{tx\.viewOnly\}/);
  assert.match(scheduleI18n,/connectionSaved:'Connection unavailable\. Showing'/);
  assert.match(scheduleI18n,/savedSchedule:'Saved schedule'/);
