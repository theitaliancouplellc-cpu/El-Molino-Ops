...
import { basicConversationAnswer, contextualSearchQuestion, isCapability, isCapabilityFollowup, isContinuation, type ConversationHistoryMessage } from '../lib/ask-conversation.ts';
...
test('130 ordinary human turns never fall into unknown restaurant-fact fallback',()=>{
  ...
  for(const q of humanTurns){
    const recognized=Boolean(basicConversationAnswer(q,history)||isContinuation(q)||isCapability(q)||isCapabilityFollowup(q,history));
    assert.equal(recognized,true,`unrecognized conversational turn: ${q}`);
  }
});

test('basic small talk gets a natural local reply',()=>{
  for(const q of ['Hi','hey there','good day','how are you doing?','thank you so much','gotcha','sounds great','hold on','see you later']){
    const answer=basicConversationAnswer(q,history);
    assert.ok(answer,`expected local reply for ${q}`);
    assert.doesNotMatch(String(answer),/not enough verified/i);
  }
});
...
test('continuation phrases preserve the previous user topic for degraded fallback',()=>{
  ...
  for(const q of ['more','go deeper','can you expand on that','what do you mean','continue','and then']){
    const contextual=contextualSearchQuestion(q,h);
    assert.match(contextual,/How does Task Center work\?/);
    assert.match(contextual,new RegExp(q.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'i'));
  }
});

test('capability wording variants are recognized',()=>{
  for(const q of ['can you help me','what else can you do','what are you able to do','tell me what you can do','what can this bot do','what can Ask AI do'])assert.equal(isCapability(q),true,q);
});

test('capability followups remain contextual',()=>{
  for(const q of ['more','tell me more about that','go deeper','can you expand on that','what do you mean'])assert.equal(isCapabilityFollowup(q,history),true,q);
});
