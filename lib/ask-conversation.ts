...
const shortAnswerPatterns=[/^(yes|yeah|yep|yup|yes please|yeah please|yep please|absolutely|definitely)$/, /^(no|nope|nah|no thanks|not really|not yet)$/, /^(maybe|maybe later|probably|possibly|not sure|i don'?t know|idk)$/];
...
const continuationPatterns=[/^(go for it|do it|go ahead|continue|keep going|carry on|proceed|move on)$/, /^(next|next step|what'?s next|whats next|then what|and then|so now what|what now|what next)$/, /^(more|say more|tell me more( about that)?|go deeper|explain more|break that down|elaborate)$/, /^(can you (?:go into|give me) (?:more )?details?(?: about that)?|go into (?:more )?details?(?: about that)?)$/, /^(can you expand on that|expand on that|can you explain that|what do you mean|what (?:all )?does that mean|how so|why is that|why|how)$/, /^(give me an example|can you give me an example|show me an example|for example|like what|such as what)$/, /^(what about|how about) .+$/];
...
const explicitOffDomain=/\b(weather|nba|nfl|mlb|nhl|football|basketball|baseball|soccer|hockey|sports?|car|cars|truck|trucks|mustang|vehicle|vehicles|engine|horsepower|anime|movie|movies|celebrity|celebrities|bitcoin|crypto|stock market|election|president|politics|quantum physics|space travel)\b/;

function capabilityContinuationAnswer(s:string,history:ConversationHistoryMessage[]){
  if(!history.length||!any(s,continuationPatterns))return null;
  const prev=lastAssistant(history);const p=norm(prev);
  const capabilityContext=/(i can answer questions|in practical terms i can help|el molino ops is organized|task center|knowledge studio|capture studio|today work team ask ai|operations app)/.test(p);
  if(!capabilityContext)return null;
  if(any(s,[/^(give me an example|can you give me an example|show me an example|for example|like what|such as what)$/]))return `For example, you could say: “Build me a closing checklist for tonight.” I can draft the checklist for review. Or ask, “What tasks are still open?”, “Where do I find employee training?”, “Explain this approved procedure,” or “Save this manager note as knowledge.” I can use the conversation we’re already having, so you can then say “make it shorter,” “add managers,” or “turn that into a task” without starting over.`;
  if(any(s,[/^(what do you mean|what (?:all )?does that mean|can you explain that|break that down|explain more|how so)$/]))return `It means the app is split into practical work areas. Today shows what matters now. Work handles tasks, checklists and procedures. Team covers employees, roles and training. Ask AI is the conversational assistant you’re using now. More contains deeper tools such as Calendar, Knowledge Studio, Menu Catalog, files, discussions, Capture Studio, account/security and Admin Center. I can explain or help use any of those areas, and I should keep track of what we’re talking about when you ask a follow-up.`;
  if(any(s,[/^(more|say more|tell me more( about that)?|go deeper|elaborate)$/, /^(can you (?:go into|give me) (?:more )?details?(?: about that)?|go into (?:more )?details?(?: about that)?)$/, /^(can you expand on that|expand on that)$/]))return `More specifically, I can help you run the restaurant through the app: find information, explain approved procedures, plan shifts, draft operational tasks and checklists, organize training or manager notes, navigate features, and turn a conversation into a reviewable task, procedure or knowledge record. I can also continue from prior messages instead of making you restate the subject each time.`;
  if(/^(why|why is that)$/.test(s))return `Because Ask AI is meant to work like an operations assistant, not just a search box. Keeping the prior conversation lets you refine an answer naturally, while restricting restaurant facts to approved internal information helps prevent the assistant from inventing operational details.`;
  return null;
}
...
export function isCapabilityFollowup(q:string,history:ConversationHistoryMessage[]){if(!isContinuation(q))return false;const recent=history.slice(-8).map(m=>norm(m.content)).join(' ');return /(what.*can you do|capabilit|answer questions|tasks|procedures|what can this bot do|what can ask ai do)/.test(recent);}
export function clearlyUnrelated(q:string,history:ConversationHistoryMessage[]=[]){const s=norm(q);if(!s||domainWords.test(s)||isContinuation(q)||isCapability(q))return false;const recent=history.slice(-8).map(m=>norm(m.content)).join(' ');if(domainWords.test(recent)&&!/^(tell me about|what is|who is|how does|explain) /.test(s))return false;return explicitOffDomain.test(s);}
export function contextualSearchQuestion(q:string,history:ConversationHistoryMessage[]){if(!isContinuation(q)||!history.length)return q;const previous=[...history].reverse().find(m=>m.role==='user'&&norm(m.content)&&!isContinuation(m.content));return previous?`${previous.content} — follow-up: ${q}`:q;}
