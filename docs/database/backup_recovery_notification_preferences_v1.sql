-- Applied to production Supabase as migration: backup_recovery_notification_preferences_v1
-- Keeps browser backup manifest and transactional server recovery allow-list aligned.
...
as $$
 select array[
  'locations','profiles',
  'employee_roles','employees','employee_role_assignments','employee_self_setup_claims','employee_self_setup_role_claims','employee_schedule_profiles','employee_availability','availability_change_requests','employee_availability_overrides','employee_compliance_profiles',
  'areas','stations','knowledge_sources','knowledge_items','procedures','procedure_steps',
  ...
  'files','import_jobs','import_rows','restaurant_performance_targets','restaurant_daily_performance','cash_control_sessions',
  'tasks','task_dependencies','comments','ops_records','calendar_events','discussion_rooms','discussion_messages','entity_file_links',
  'activity_log','notifications','notification_preferences','mentions','favorites','recent_views','saved_views','dashboard_widgets','ai_conversations','ai_messages','user_preferences','user_invitations','content_versions','food_safety_temperature_points',
  ...
  'onboarding_packages','onboarding_documents','onboarding_package_items','onboarding_assignments','onboarding_item_progress','onboarding_acknowledgments','onboarding_comments'
 ]::text[]
$$;
