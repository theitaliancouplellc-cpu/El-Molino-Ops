...
type UndoState={label:string;id:string;patch:Record<string,unknown>}|null;
...
export default function OpsPage(){
  ...
  async function save(e:FormEvent){e.preventDefault();if(!profile?.location_id)return;const clean=normalizeOpsData(kind,draft.data);const errors=validateOpsRecord(kind,draft.title,clean);if(errors.length){flash(errors[0]);return;}const duplicate=rows.find(r=>r.kind===kind&&r.id!==editing?.id&&normalizedTitle(r.title)===normalizedTitle(draft.title)&&!r.deleted_at);if(duplicate)return flash(`A ${module.title.toLowerCase().replace(/s$/,'')} named “${duplicate.title}” already exists.`);setBusy(true);const {data:u}=await supabase.auth.getUser();if(!u.user){setBusy(false);return;}const payload={location_id:profile.location_id,kind,title:draft.title.trim(),priority:draft.priority,sensitivity:module.sensitivity,data:clean,due_at:draft.due_at?new Date(draft.due_at).toISOString():null,created_by:u.user.id};let result;if(editing){const {created_by,...update}=payload;result=await supabase.from('ops_records').update({...update,updated_by:u.user.id}).eq('id',editing.id).select('id').single()}else result=await supabase.from('ops_records').insert(payload).select('id').single();setBusy(false);if(result.error){flash(result.error.message);return;}await supabase.from('activity_log').insert({location_id:profile.location_id,actor_user_id:u.user.id,action:editing?'updated':'created',entity_type:'ops_record',entity_id:result.data.id,summary:`${editing?'Updated':'Created'} ${module.title}: ${draft.title.trim()}`});resetForm();await load();flash(editing?'Updated.':'Saved.')}
  async function setStatus(row:Row,status:string){const prior=row.status;const {error}=await supabase.from('ops_records').update({status}).eq('id',row.id);if(error)return flash(error.message);setUndo({label:`Undo status change for ${row.title}`,id:row.id,patch:{status:prior}});await load();flash(`Marked ${status}.`)}
  async function archive(row:Row){const prior=row.archived_at;const next=row.archived_at?null:new Date().toISOString();const {error}=await supabase.from('ops_records').update({archived_at:next}).eq('id',row.id);if(error)return flash(error.message);setUndo({label:`Undo ${row.archived_at?'restore':'archive'} for ${row.title}`,id:row.id,patch:{archived_at:prior}});await load();flash(row.archived_at?'Restored from archive.':'Archived.')}
  async function remove(row:Row){if(!confirm(`Move “${row.title}” to trash?`))return;const {error}=await supabase.from('ops_records').update({deleted_at:new Date().toISOString()}).eq('id',row.id);if(error)return flash(error.message);setUndo({label:`Restore ${row.title}`,id:row.id,patch:{deleted_at:null}});await load();flash('Moved to trash.')}
  async function applyUndo(){if(!undo)return;const current=undo;const {error}=await supabase.from('ops_records').update(current.patch).eq('id',current.id);if(error)return flash(error.message);setUndo(null);await load();flash('Change undone.')}
  ...
  return <div className="app-shell">
    ...
    <main className="page">
      {message&&<div className="toast-message">{message}</div>}{undo&&<div className="notice" style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,marginBottom:12}}><span>{undo.label}</span><button className="mini-action" onClick={applyUndo}><Undo2 size={14}/> Undo</button></div>}
      ...
      {showForm&&<form className="card form" onSubmit={save} style={{marginBottom:16}}>
        <div className="card-title-row"><h3>{editing?'Edit':'New'} {module.title.replace(/s$/,'')}</h3><button type="button" className="icon-action" onClick={resetForm} aria-label="Close form"><X/></button></div>
