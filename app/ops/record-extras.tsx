...
export default function RecordExtras({recordId,title}:{recordId:string;title:string}){
  ...
  async function toggleSaved(){
    ...
    if(saved){
      ...
      if(error)return setMessage(error.message);setSaved(false);setMessage(t('Removed from saved.','Eliminado de guardados.'));
    }else{
      ...
      if(error)return setMessage(error.message);setSaved(true);setMessage(t('Saved.','Guardado.'));
    }
  }
  ...
  async function openFile(file:Attachment){
    ...
    if(error||!data?.signedUrl)return setMessage(error?.message||t('Could not open attachment.','No se pudo abrir el archivo adjunto.'));
    window.open(data.signedUrl,'_blank','noopener,noreferrer');
  }

  if(!loaded)return <div className="row-actions" style={{marginTop:10}}><button className="mini-action" onClick={load} disabled={loading}>{loading?<Loader2 className="spin" size={14}/>:<Paperclip size={14}/>} {t('Files & saved','Archivos y guardados')}</button></div>;
  return <div style={{marginTop:12}}>
    {message&&<small style={{display:'block',marginBottom:8,color:'var(--muted)'}}>{message}</small>}
    <div className="row-actions" style={{flexWrap:'wrap'}}>
      <button className="mini-action" onClick={toggleSaved}>{saved?<StarOff size={14}/>:<Star size={14}/>} {saved?t('Unsave','Quitar de guardados'):t('Save','Guardar')}</button>
      <a className="mini-action" href={`/capture?entityType=ops_record&entityId=${recordId}`}><Paperclip size={14}/> {t('Attach file','Adjuntar archivo')}</a>
    </div>
    ...
    {!files.length&&<small style={{display:'block',marginTop:8,color:'var(--muted)'}}>{t('No attachments yet.','Aún no hay archivos adjuntos.')}</small>}
  </div>;
}
