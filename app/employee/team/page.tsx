...
export default function EmployeeTeam(){
 ...
 async function startGroup(){if(!groupTitle.trim()||!groupMembers.length||busy)return;setBusy(true);const {data,error}=await supabase.rpc('start_team_group_channel',{p_title:groupTitle.trim(),p_member_employee_ids:groupMembers,p_client_request_id:crypto.randomUUID()});if(error)setNotice(error.message);else{setGroupTitle('');setGroupMembers([]);setGroupPicker('');await load(String(data))}setBusy(false)}
 ...
 function addGroupMember(){if(!groupPicker||groupMembers.includes(groupPicker))return;setGroupMembers(x=>[...x,groupPicker]);setGroupPicker('')}
 function removeGroupMember(id:string){setGroupMembers(x=>x.filter(v=>v!==id))}
