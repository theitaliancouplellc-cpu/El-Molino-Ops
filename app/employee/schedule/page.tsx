...
export default function EmployeeSchedule(){
 ...
 async function submitSwap(){if(busy||!swapSource||!swapTarget||!requireConnection())return;const target=swapCandidates.find(s=>s.shift_id===swapTarget);if(!target)return setMessage(tx.chooseCoworker);setBusy(true);const {error}=await supabase.rpc('submit_my_shift_change_request',{p_shift_id:swapSource,p_request_type:'swap',p_target_shift_id:target.shift_id,p_reason:tx.tradeReason});setMessage(error?error.message:tx.tradeSuccess);if(!error){setSwapSource(null);setSwapTarget('');setSwapCandidates([])}setBusy(false)}
