...
export default function EmployeeSchedule(){
 ...
 async function loadNoticeContext(id:string){
  ...
  const raw=n.data?.changed_shift_ids;const ids=Array.isArray(raw)?raw.filter((x):x is string=>typeof x==='string'):[];
  setChangedShiftIds(new Set(ids));if(ids.length)setMessage(`${ids.length} updated shift${ids.length===1?' is':'s are'} highlighted below.`)
 }
 async function loadWeek(emp=employeeId,ws=weekStart){if(!emp)return;setBusy(true);setMessage('');const tz='America/New_York',from=zonedLocalToIso(ws,'00:00:00',tz),to=zonedLocalToIso(addDateDays(ws,7),'00:00:00',tz);const [p,s,all,r,e,ra,b]=await Promise.all([
  ...
  supabase.from('schedule_shifts').select('id,employee_id,role_id,starts_at,ends_at,break_minutes,status,notes,coverage_requirement_id').in('status',['scheduled','covered']).gte('starts_at',from).lt('starts_at',to).order('starts_at'),
  supabase.from('employee_roles').select('id,name').order('name'),
  supabase.from('employees').select('id,full_name').eq('active',true).is('deleted_at',null).order('full_name'),
 ...
 ]);if([p,s,all,r,e,ra,b].some(x=>x.error))setMessage('Some schedule details could not be loaded.');setPeriod((p.data??null) as Period|null);setShifts((s.data??[]) as Shift[]);setVisibleShifts((all.data??[]) as Shift[]);setRoles((r.data??[]) as Role[]);setEmployees((e.data??[]) as Employee[]);setRoleAssignments((ra.data??[]) as RoleAssignment[]);setBreaks((b.data??[]) as BreakRow[]);setSwapSource(null);setSwapTarget('');setBusy(false)}
 ...
 async function coverage(s:Shift){if(busy||!profile?.location_id)return;const reason=prompt('Why do you need coverage? (optional)')||'';setBusy(true);const {error}=await supabase.from('shift_change_requests').insert({location_id:profile.location_id,shift_id:s.id,request_type:'coverage',requested_by_employee_id:employeeId,reason:reason.trim().slice(0,2000)||null});setMessage(error?error.message:'Coverage request sent to management.');setBusy(false)}
 async function submitSwap(){if(busy||!profile?.location_id||!swapSource||!swapTarget)return;const target=visibleShifts.find(s=>s.id===swapTarget);if(!target?.employee_id)return setMessage('Choose a valid coworker shift.');setBusy(true);const {error}=await supabase.from('shift_change_requests').insert({location_id:profile.location_id,shift_id:swapSource,target_shift_id:target.id,request_type:'swap',requested_by_employee_id:employeeId,target_employee_id:target.employee_id,reason:'Employee requested shift swap'});setMessage(error?error.message:'Shift trade sent to management for approval.');if(!error){setSwapSource(null);setSwapTarget('')}setBusy(false)}
 const weekEnd=addDateDays(weekStart,6),total=useMemo(()=>shifts.reduce((a,s)=>a+shiftNetHours(s.starts_at,s.ends_at,s.break_minutes),0),[shifts]),roleName=(id:string|null)=>roles.find(r=>r.id===id)?.name||'Shift',employeeName=(id:string|null)=>employees.find(e=>e.id===id)?.full_name||'Coworker';
 const roleSet=useMemo(()=>new Set(roleAssignments.map(x=>`${x.employee_id}:${x.role_id}`)),[roleAssignments]);
 const swapCandidates=useMemo(()=>{const source=shifts.find(s=>s.id===swapSource);if(!source?.role_id)return[];return visibleShifts.filter(t=>t.id!==source.id&&t.employee_id&&t.employee_id!==employeeId&&t.role_id&&roleSet.has(`${employeeId}:${t.role_id}`)&&roleSet.has(`${t.employee_id}:${source.role_id}`)&&new Date(t.ends_at)>new Date())},[swapSource,shifts,visibleShifts,employeeId,roleSet]);
