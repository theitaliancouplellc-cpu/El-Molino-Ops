...
export default function EmployeeSchedule(){
 ...
 async function submitSwap(){if(busy||!profile?.location_id||!swapSource||!swapTarget)return;const target=swapCandidates.find(s=>s.shift_id===swapTarget);if(!target)return setMessage('Choose a valid coworker shift.');setBusy(true);const {error}=await supabase.from('shift_change_requests').insert({location_id:profile.location_id,shift_id:swapSource,target_shift_id:target.shift_id,request_type:'swap',requested_by_employee_id:employeeId,target_employee_id:target.employee_id,reason:'Employee requested shift swap'});setMessage(error?error.message:'Shift trade sent to management for approval.');if(!error){setSwapSource(null);setSwapTarget('');setSwapCandidates([])}setBusy(false)}
 ...
 return <main className={styles.page}>
  ...
  {swapSource&&<section className={styles.section}><div className={styles.setupCard}><h2>Trade this shift</h2><p className={styles.muted}>Only reciprocal matches that preserve role qualifications, avoid approved time off, and avoid obvious schedule overlaps are shown. Management still approves the final trade.</p><label className={styles.field}><span>Coworker shift</span><select value={swapTarget} onChange={e=>setSwapTarget(e.target.value)}><option value="">Choose an eligible shift</option>{swapCandidates.map(s=><option key={s.shift_id} value={s.shift_id}>{s.employee_name} · {s.role_name} · {new Date(s.starts_at).toLocaleString([],{weekday:'short',month:'short',day:'numeric',hour:'numeric',minute:'2-digit'})}</option>)}</select></label>{!swapCandidates.length&&<div className={styles.empty}>No reciprocal trade matches are available for this shift right now.</div>}<div className={styles.actions}><button className={styles.button} disabled={busy||!swapTarget} onClick={submitSwap}>Submit Trade</button><button className={`${styles.button} ${styles.secondary}`} onClick={()=>{setSwapSource(null);setSwapTarget('');setSwapCandidates([])}}>Cancel</button></div></div></section>}
