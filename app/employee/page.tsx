...
export default function EmployeeHome(){
 ...
 function attentionFor(p:Priority){
  if(p.top_priority==='announcement_ack')return {title:t('employee.attnAckTitle'),body:`${p.required_acknowledgments} ${t('employee.attnAckBody')}`,href:'/employee/team',label:t('employee.required')};
  if(p.top_priority==='urgent_announcement')return {title:t('employee.attnUrgentTitle'),body:`${p.urgent_announcements} ${t('employee.attnUrgentBody')}`,href:'/employee/team',label:t('employee.urgent')};
  if(p.top_priority==='training_overdue')return {title:t('employee.attnTrainingTitle'),body:`${p.training_overdue} ${t('employee.attnTrainingBody')}`,href:'/employee/training',label:t('employee.overdue')};
  if(p.top_priority==='team_message')return {title:t('employee.attnMessageTitle'),body:`${p.unread_messages} ${t('employee.attnMessageBody')}`,href:'/employee/team',label:t('employee.message')};
  if(p.top_priority==='pending_request')return {title:t('employee.attnRequestTitle'),body:`${p.pending_requests} ${t('employee.attnRequestBody')}`,href:'/employee/requests',label:t('employee.pending')};
  return null;
 }
 ...
 return <main className={styles.page}>
  ...
  <section className={styles.hero}><small>{t('employee.app')}</small><h1>{t('employee.hello')}, {first}.</h1><p>{t('employee.heroBody')}</p>{next?<div className={styles.next}><span>{t('employee.nextShift')} · {countdown(next.starts_at)}</span><b>{roleName(next.role_id)} · {new Date(next.starts_at).toLocaleDateString(dateLocale,{weekday:'long',month:'short',day:'numeric'})}</b><span>{new Date(next.starts_at).toLocaleTimeString(dateLocale,{hour:'numeric',minute:'2-digit'})} – {new Date(next.ends_at).toLocaleTimeString(dateLocale,{hour:'numeric',minute:'2-digit'})} · {hours(next).toFixed(1)} {t('common.hours')} · {next.break_minutes?`${next.break_minutes} ${t('common.minutes')}`:t('common.noScheduledBreak')}</span>{next.notes&&<span>{t('common.shiftNote')} · {next.notes}</span>}</div>:<div className={styles.next}><b>{t('employee.noUpcoming')}</b><span>{t('employee.noUpcomingBody')}</span></div>}</section>
  ...
  <section className={styles.section}><div className={styles.sectionHead}><h2>{t('employee.yourApp')}</h2><span>{shifts.length} {t('employee.upcoming')} · {upcomingHours.toFixed(1)} {t('common.hours')}</span></div><div className={styles.grid}>
   <a className={styles.card} href="/employee/schedule"><CalendarDays/><b>{t('employee.mySchedule')}</b><small>{t('employee.myScheduleBody')}</small></a>
   <a className={styles.card} href="/employee/shift-pool"><Repeat2/><b>{t('employee.shiftPool')}</b><small>{t('employee.shiftPoolBody')}</small></a>
   <a className={styles.card} href="/employee/requests"><Clock3/><b>{t('employee.availability')}</b><small>{t('employee.availabilityBody')}</small></a>
   <a className={styles.card} href="/employee/team"><MessageSquare/><b>{t('employee.teamHub')}</b><small>{t('employee.teamHubBody')}</small></a>
   <a className={styles.card} href="/employee/training"><GraduationCap/><b>{t('nav.training')}</b><small>{t('employee.trainingBody')}</small></a>
   <a className={styles.card} href="/employee/time-clock"><Clock3/><b>{t('nav.timeClock')}</b><small>{t('employee.timeClockBody')}</small></a>
   <a className={styles.card} href="/employee/tips"><Coins/><b>{t('account.myTips')}</b><small>{t('employee.myTipsBody')}</small></a>
  ...
  <nav className={styles.tabs} aria-label={t('employee.staffNav')}><a className={`${styles.tab} ${styles.tabActive}`} href="/employee"><Home size={19}/>{t('nav.home')}</a><a className={styles.tab} href="/employee/schedule"><CalendarDays size={19}/>{t('nav.schedule')}</a><a className={styles.tab} href="/employee/requests"><Clock3 size={19}/>{t('nav.requests')}</a><a className={styles.tab} href="/employee/team"><MessageSquare size={19}/>{t('nav.team')}</a><a className={styles.tab} href="/account"><UserRound size={19}/>{t('common.more')}</a></nav>
 </main>
}
