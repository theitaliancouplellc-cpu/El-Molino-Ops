...
export default function TeamSetupPage(){
 ...
 async function load(){
  ...
  if(!p.data||!['admin','manager'].includes(p.data.app_role)){location.href='/employee';return}
  ...
  const [c,r,rc,e,i]=await Promise.all([
  ...
  if(c.error||r.error||rc.error||e.error||i.error)setMessage(c.error?.message||r.error?.message||rc.error?.message||e.error?.message||i.error?.message||'Could not load team setup.');
  const cs=(c.data??[]) as Claim[],rr=(rc.data??[]) as RoleClaim[];setClaims(cs);setRoles((r.data??[]) as Role[]);setEmployees((e.data??[]) as Employee[]);setInvites((i.data??[]) as Invite[]);const map:Record<string,string[]>={};for(const claim of cs)map[claim.id]=rr.filter(x=>x.claim_id===claim.id).map(x=>x.role_id);setSelectedRoles(map);setBusy(false);setReady(true)
 }
 ...
 return <main className={styles.page}>
  <div className={styles.top}><div><h1>Employee App Setup</h1><p>Invite staff, then verify every profile and job role before schedule publishing begins.</p></div><Link className={styles.back} href="/manager">Back to Manager</Link></div>
