...
import { getJohnsIslandWeatherContext, needsLiveWeather } from '../../../lib/live-weather';
...
const FREE_MODELS=['zai/glm-4.6v-flash','poolside/laguna-s-2.1-free','inclusionai/ling-3.0-tiny-free'];
...
function parseActionText(text:string,type:ActionProposal['type']){try{const first=text.indexOf('{'),last=text.lastIndexOf('}');if(first<0||last<=first)return {answer:plainAssistantText(text),action:null};const parsed=JSON.parse(text.slice(first,last+1));const a=parsed?.action;const title=cleanTitle(a?.title,200);if(!title)return {answer:plainAssistantText(parsed?.answer||text),action:null};return {answer:plainAssistantText(parsed.answer||`I drafted that ${type}. Review it before creating it.`),action:{type,title,description:clean(a.description,8000).trim()||undefined,content:clean(a.content,12000).trim()||undefined,priority:['low','normal','high','urgent'].includes(a.priority)?a.priority:'normal'} as ActionProposal};}catch{return {answer:plainAssistantText(text),action:null};}}

export async function GET(){return NextResponse.json({ok:true,mode:'model-first-free-ai',providers:configuredFreeProviders(),models:FREE_MODELS});}
...
export async function POST(req:Request){
  try{
    ...
    const weatherContext=needsLiveWeather(retrievalQuery)?await getJohnsIslandWeatherContext():'';
    const liveContext=weatherContext?{weather:weatherContext}:undefined;
    ...
    const system=`You are Ask El Molino, a full conversational AI assistant for El Molino and the El Molino Ops app. Behave as a fully conversational AI assistant while staying inside that scope. Respond naturally, not like a search engine, form, scripted bot or database browser. Every user message is part of one continuous conversation. Understand greetings, slang, incomplete thoughts, pronouns, follow-ups, corrections, jokes, planning questions, explanations, brainstorming and ordinary conversation from meaning and history rather than phrase matching. Your scope is El Molino only: the restaurant, its operations, staff, guests, service, sales, costs, marketing, training, scheduling, procedures, menu, records, app data and work that directly supports El Molino. Current Johns Island weather is in scope because it affects restaurant operations. When live weather context is supplied, answer weather questions directly from it. Do not claim you cannot access weather, do not ask the user to provide weather data, and do not ask for a location when the restaurant location is already Johns Island. Do not answer unrelated general-knowledge questions and do not browse or research unrelated topics. If a request is outside El Molino scope, respond with one short sentence only stating that you are focused on El Molino. Do not explain what information or access you lack. Do not offer to help find the unrelated answer, ask for more unrelated details, suggest another source, or invent a restaurant-related use for the unrelated topic. Stop after the refusal. Use the supplied app reference and approved internal context as your authoritative private context for El Molino facts. Do not expose raw retrieval blocks or say 'from approved knowledge I found' unless the user explicitly asks for sources. Synthesize relevant internal information naturally into the answer. If internal context does not establish a restaurant-specific fact, say you do not have that verified detail rather than inventing it. Never invent completed app actions. Output plain text only. Do not use Markdown, asterisks, hashtags, backticks, bold markers or decorative symbols. Use normal sentences and short paragraphs; only use a simple numbered list when the user clearly asks for a list. ${actionInstruction}`;
    const messages:AIMessage[]=[{role:'system',content:system},...history,{role:'user',content:`${question}\n\nPRIVATE APP REFERENCE (use only when relevant):\n${APP_KNOWLEDGE}\n\nPRIVATE LIVE JOHNS ISLAND WEATHER (use directly when relevant):\n${weatherContext||'(not requested or unavailable)'}\n\nPRIVATE RELEVANT APPROVED EL MOLINO KNOWLEDGE (use naturally; do not dump verbatim):\n${context||'(none)'}\n\nPRIVATE RELEVANT APPROVED PROCEDURES (use naturally; do not dump verbatim):\n${proc||'(none)'}`}];

    process.env.EL_MOLINO_AGENT_MODEL=FREE_MODELS[0];
    process.env.EL_MOLINO_AGENT_FALLBACK_MODELS=FREE_MODELS.slice(1).join(',');
    const result=await runFreeAI(messages);
    if(!result){if(actionType){const action=localActionProposal(question,actionType);return NextResponse.json({answer:'The AI model is temporarily unavailable, but I saved a local draft for you to review instead of pretending the action was completed.',action,citations:[],degraded:true,local:true,liveContext});}return NextResponse.json({answer:'The AI model is temporarily unavailable right now. Try again in a moment.',citations:[],degraded:true,liveContext},{status:200});}
    if(actionType){const resultAction=parseActionText(result.text,actionType);if(!resultAction.action){const action=localActionProposal(question,actionType);return NextResponse.json({answer:'I understood the request, but the model response could not be converted into a safe action. I prepared a local draft for review instead.',action,citations:[],degraded:true,liveContext});}return NextResponse.json({answer:resultAction.answer,action:resultAction.action,citations:[],ai:{provider:result.provider,model:result.model}});}
    return NextResponse.json({answer:plainAssistantText(result.text),citations:[],ai:{provider:result.provider,model:result.model}});
